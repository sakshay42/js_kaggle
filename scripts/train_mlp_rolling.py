...
MODEL_DIR = PROJECT_ROOT / "models" / "neural" / "rolling_mlp"
LOG_DIR = PROJECT_ROOT / "logs"

TRAIN_PATH = DATA_DIR / "train.parquet"
VALID_PATH = DATA_DIR / "valid.parquet"
TEST_PATH = DATA_DIR / "test.parquet"
REPORT_PATH = LOG_DIR / "train_mlp_rolling_report.json"
BEST_MODEL_PATH = MODEL_DIR / "best_mlp.pt"
...
class TabularDataset(Dataset):
    def __init__(self, df, numeric_cols, cat_maps, scaler=None, fit_scaler=False):
        ...
        for col in EMBED_COLS:
            ...
            cats.append(np.array([mapping.get(int(v), 0) for v in values], dtype=np.int64))
...
def main():
    ...
    train_ds = TabularDataset(train_df, numeric_cols, cat_maps, fit_scaler=True)
    valid_ds = TabularDataset(valid_df, numeric_cols, cat_maps, scaler=train_ds.scaler)
    test_ds = TabularDataset(test_df, numeric_cols, cat_maps, scaler=train_ds.scaler)
    ...
    pred_df = pd.concat(
        [
            valid_df[[DATE_COL, "time_id", "symbol_id", TARGET_COL, WEIGHT_COL]].assign(split="valid", mlp_pred=valid_pred),
            test_df[[DATE_COL, "time_id", "symbol_id", TARGET_COL, WEIGHT_COL]].assign(split="test", mlp_pred=test_pred),
        ],
        ignore_index=True,
    )
