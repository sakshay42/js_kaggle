...
MODEL_DIR = PROJECT_ROOT / "models" / "neural" / "regularized_mlp_focused"
LOG_DIR = PROJECT_ROOT / "logs"

TRAIN_PATH = DATA_DIR / "train.parquet"
VALID_PATH = DATA_DIR / "valid.parquet"
TEST_PATH = DATA_DIR / "test.parquet"
REPORT_PATH = LOG_DIR / "train_regularized_mlp_focused_report.json"
...
EPOCHS = 7
PATIENCE = 2
SHRINK_GRID = [0.40, 0.50, 0.60, 0.75, 0.90, 1.00]
SEEDS = [42, 202, 777]


BASE_CONFIGS = [
    {
        "base_name": "small_no_embed",
        "hidden": [128, 64],
        "dropout": 0.25,
        "lr": 3e-4,
        "weight_decay": 1e-3,
    ...
    {
        "base_name": "small_no_embed_more_dropout",
        "hidden": [128, 64],
        "dropout": 0.35,
        "lr": 3e-4,
        "weight_decay": 1e-3,
    ...
    {
        "base_name": "small_no_embed_more_decay",
        "hidden": [128, 64],
        "dropout": 0.25,
        "lr": 3e-4,
        "weight_decay": 3e-3,
...
def expand_configs():
    ...
    for base in BASE_CONFIGS:
        for seed in SEEDS:
            cfg = dict(base)
            cfg["seed"] = seed
            cfg["name"] = f"{base['base_name']}_seed{seed}"
            configs.append(cfg)
    return configs
...
def choose_shrink(y, pred, w):
    rows = [{"shrink": s, "valid_weighted_r2": weighted_r2(y, s * pred, w)} for s in SHRINK_GRID]
    return max(rows, key=lambda x: x["valid_weighted_r2"])
...
def train_config(config, train_df, valid_df, test_df, numeric_cols, cat_maps, cat_sizes, scaler, device):
    ...
    model = RegularizedMLP(
        len(numeric_cols), cat_sizes, config["hidden"], config["dropout"], config["use_embeddings"]
    ).to(device)
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        raw_score = weighted_r2(valid_y, valid_pred, valid_w)
        shrink_row = choose_shrink(valid_y, valid_pred, valid_w)
        row = {
            "model": config["name"],
            "epoch": epoch,
            "train_loss": float(train_loss),
            "valid_weighted_r2": float(raw_score),
            "best_shrink": float(shrink_row["shrink"]),
            "best_shrunk_valid_weighted_r2": float(shrink_row["valid_weighted_r2"]),
        }
        history.append(row)
        print(row, flush=True)
        if shrink_row["valid_weighted_r2"] > best_score:
            best_score = shrink_row["valid_weighted_r2"]
            best_epoch = epoch
            best_shrink = shrink_row["shrink"]
    ...
    valid_score = weighted_r2(valid_y, best_shrink * valid_pred, valid_w)
    test_score = weighted_r2(test_y, best_shrink * test_pred, test_w)
    ...
    preds = pd.concat(
        [
            valid_df[[DATE_COL, "time_id", "symbol_id", TARGET_COL, WEIGHT_COL]].assign(
                split="valid", model=config["name"], prediction=best_shrink * valid_pred
            ),
            test_df[[DATE_COL, "time_id", "symbol_id", TARGET_COL, WEIGHT_COL]].assign(
                split="test", model=config["name"], prediction=best_shrink * test_pred
            ),
        ],
        ignore_index=True,
    )
...
def main():
    ...
    summaries, histories, pred_frames = [], [], []
